...
Each agent in the handoff chain calls the Responses API independently. All calls are traced individually. Open the session in **Sessions** to see the full conversation in order.
...
agent = Agent(
    name="OrderDesk",
    instructions="Look up order status when asked.",
