...
Provider credentials are configured in the proxy and resolved server-side. So is the model: the `model` field in your request is replaced with the model configured on the proxy. Use your framework's OpenAI-compatible client for every provider, including Anthropic, Groq, Bedrock, and Gemini; provider-native SDKs use different API paths and cannot target the proxy.
...
<Warning>
The SDK's hosted decision endpoint (`POST /v1/execute`) is not live yet, so SDK mode cannot evaluate policy today. See the [SDK page](/integrations/xenovia-sdk) for the exact behaviour of each `unreachable_mode`.
</Warning>
...
- If it only needs HTTP access, point it at `https://runtime.xenovia.io/{proxy_id}/v1`.
