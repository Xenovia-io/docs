...
Pass `X-Xenovia-Session-Id` (a valid UUID) to group related calls into a session. All turns under that session ID appear together in **Sessions** with a per-session turn counter.
...
with httpx.Client() as http:
    resp = http.post(
        f"https://runtime.xenovia.io/{os.environ['XENOVIA_AGENT_ID']}/v1/chat/completions",
        headers={
