...
The `/v1` suffix exposes the two OpenAI-compatible routes the runtime serves today: `/v1/chat/completions` and `/v1/responses`. Any other path under the proxy — `/v1/embeddings`, `/v1/completions`, `/v1/models`, provider-prefixed paths such as `/openai/v1/...`, and the legacy `/a/{proxy_id}/...` prefix — returns `404 Not Found`.
