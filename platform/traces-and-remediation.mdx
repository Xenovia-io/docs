...
The [management API](/api-reference/introduction) exposes sessions, turns, aggregate stats, and multi-proxy workflows. The `GET /api/v1/traces` endpoint filters by proxy, session, decision, provider, and time range, and by any custom property as `cp_<key>=<value>`.
